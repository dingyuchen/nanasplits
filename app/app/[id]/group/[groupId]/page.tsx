...
export default async function GroupPage({
  ...
  const { isMember } = await checkMembership(groupIdNumber, userId);
  if (!isMember) {
    return (
        ...
        <div className="text-center max-w-md">
          ...
          <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl p-4">
            <p className="text-sm text-blue-900 dark:text-blue-200">
              💡 To join this group, click the "Get Started" button in the
              Telegram group chat.
            </p>
          </div>
        </div>
  ...
  const groupData = await fetchQuery(api.groups.getListOfExpenses, {
    telegramChatId: groupIdNumber,
  });
  if (!groupData) {
    return (
      <div className="min-h-screen bg-gradient-to-b from-blue-50 to-white dark:from-gray-900 dark:to-gray-800 flex items-center justify-center p-6 text-center">
        <div>
  ...
  const isRegisteredMemberOfGroup = await fetchQuery(
          ...
          <div className="space-y-3">
            {expenses.length === 0 ? (
              ...
              expenses.map((expense) => {
                ...
                const isMe = payer?.telegramUserId === userId;
